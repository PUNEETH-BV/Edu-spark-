...
export const supabase = {
  auth: {
    async signUp({ email, password, options }) {
      ...
      localStorage.setItem('session_meta', JSON.stringify({ user, expiresAt: Date.now() + 7 * 24 * 60 * 60 * 1000 }));
    ...
    async signInWithPassword({ email, password }) {
      ...
      localStorage.setItem('session_meta', JSON.stringify({ user: safeUser, expiresAt: Date.now() + 7 * 24 * 60 * 60 * 1000 }));
    ...
    async getUser() {
      if (typeof window === 'undefined') return { data: { user: null }, error: null };
      const meta = JSON.parse(localStorage.getItem('session_meta') || 'null');
    ...
    async signInWithOAuth({ provider, options }) {
      ...
      if (provider === 'google') {
        ...
        if (!email || !email.trim()) {
          return { data: null, error: { message: 'Google login cancelled' } };
        }
        ...
        let profiles = JSON.parse(localStorage.getItem('db_profiles') || '[]');
        const userProfileId = existingUser ? existingUser.id : googleUser.id;
        let profile = profiles.find(p => p.id === userProfileId);
        if (!profile) {
          profile = {
            id: userProfileId,
            username: cleanName,
        ...
        const finalUser = existingUser ? { id: existingUser.id, email: existingUser.email } : googleUser;
        ...
        localStorage.setItem('session_user', JSON.stringify(finalUser));
        localStorage.setItem('session_meta', JSON.stringify(sessionData));
...
if (typeof window !== 'undefined') {
  ...
  if (!isSeeded) {
    ...
    // No auto-login on first load (user must authenticate)
